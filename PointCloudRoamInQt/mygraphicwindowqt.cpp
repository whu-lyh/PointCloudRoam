...
MyGraphicWindowQt::MyGraphicWindowQt( osg::GraphicsContext::Traits* traits, QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f )
:   _realized(false)
{
...
MyGraphicWindowQt::MyGraphicWindowQt( GLWidget* widget )
:   _realized(false)
{
    _widget = widget;
...
MyGraphicWindowQt::~MyGraphicWindowQt()
{
    close();
...
bool MyGraphicWindowQt::init( QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f )
{
    ...
    if ( !_widget )
    {
        // shareWidget
        if ( !shareWidget ) {
            MyGraphicWindowQt* sharedContextQt = dynamic_cast<MyGraphicWindowQt*>(_traits->sharedContext.get());
            if ( sharedContextQt )
...
QGLFormat MyGraphicWindowQt::traits2qglFormat( const osg::GraphicsContext::Traits* traits )
{
...
void MyGraphicWindowQt::qglFormat2traits( const QGLFormat& format, osg::GraphicsContext::Traits* traits )
{
...
osg::GraphicsContext::Traits* MyGraphicWindowQt::createTraits( const QGLWidget* widget )
{
...
bool MyGraphicWindowQt::setWindowRectangleImplementation( int x, int y, int width, int height )
{
    if ( _widget == NULL )
        return false;
...
void MyGraphicWindowQt::getWindowRectangle( int& x, int& y, int& width, int& height )
{
    if ( _widget )
    {
...
bool MyGraphicWindowQt::setWindowDecorationImplementation( bool windowDecoration )
{
...
bool MyGraphicWindowQt::getWindowDecoration() const
{
    return _traits->windowDecoration;
}

void MyGraphicWindowQt::grabFocus()
{
    if ( _widget )
        _widget->setFocus( Qt::ActiveWindowFocusReason );
}

void MyGraphicWindowQt::grabFocusIfPointerInWindow()
{
    if ( _widget->underMouse() )
        _widget->setFocus( Qt::ActiveWindowFocusReason );
}

void MyGraphicWindowQt::raiseWindow()
{
    if ( _widget )
        _widget->raise();
}

void MyGraphicWindowQt::setWindowName( const std::string& name )
{
    if ( _widget )
        _widget->setWindowTitle( name.c_str() );
}

std::string MyGraphicWindowQt::getWindowName()
{
    return _widget ? _widget->windowTitle().toStdString() : "";
}

void MyGraphicWindowQt::useCursor( bool cursorOn )
{
    if ( _widget )
    {
...
void MyGraphicWindowQt::setCursor( MouseCursor cursor )
{
...
bool MyGraphicWindowQt::valid() const
{
    return _widget && _widget->isValid();
}

bool MyGraphicWindowQt::realizeImplementation()
{
...
bool MyGraphicWindowQt::isRealizedImplementation() const
{
    return _realized;
}

void MyGraphicWindowQt::closeImplementation()
{
    if ( _widget )
        _widget->close();
    _realized = false;
...
void MyGraphicWindowQt::runOperations()
{
...
bool MyGraphicWindowQt::makeCurrentImplementation()
{
    if (_widget->getNumDeferredEvents() > 0)
        _widget->processDeferredEvents();
...
bool MyGraphicWindowQt::releaseContextImplementation()
{
    _widget->doneCurrent();
    return true;
}

void MyGraphicWindowQt::swapBuffersImplementation()
{
...
void MyGraphicWindowQt::requestWarpPointer( float x, float y )
{
    if ( _widget )
        QCursor::setPos( _widget->mapToGlobal(QPoint((int)x,(int)y)) );
}

void MyGraphicWindowQt::setMainWindow(MainWindow *mainWindow)
{
    _mainWindow=mainWindow;
}

void MyGraphicWindowQt::onMoveOpenglContextToNewThread(QThread *newThread)
{
    QOpenGLContext* qglcx = _widget->context()->contextHandle();
...
public:
    ...
    virtual osg::GraphicsContext* createGraphicsContext( osg::GraphicsContext::Traits* traits )
    {
        ...
        {
            osg::ref_ptr< MyGraphicWindowQt > window = new MyGraphicWindowQt( traits );
