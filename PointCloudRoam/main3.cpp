...
int main (int argc, char **argv)
{
	...
	std::string pointfilepathorigin = "D:/data/Estonia/roam/20200309/origin2";
	std::string pointfilepathrefine = "D:/data/Estonia/roam/20200309/refine2";
	std::string traj_file = "D:/data/Estonia/roam/20200309/Estonia.traj";//osg roam only need a few of view points which is different from others
	//traj file fromat should be emphasized
	...
	//viewer origin
	{
		...
		if ( !loadTraj ( traj_file, animation_path, osg::Vec3d () ) )
		{
			std::cerr << "The traj file load failed,Please check it";
			comptviewer->done ();
			std::system ( "pause" );
			return 0;
		}
	...
	const osg::Vec3d& offset)
{
	...
	if (!ifs)
	{
		std::cerr << "The traj file open fails. Please check it" << std::endl;
		return false;
	}
	int num_pts;
	ifs >> num_pts;
	while (!ifs.eof ())
	{
		...
		{
			std::cerr << "The traj file format is incorrect. Please check it" << std::endl;
			return false;
		}
